  ...
  return (
      ...
      <Form {...form}>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 flex flex-col gap-5 w-420 mt-4">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input type="email" placeholder="请输入用户名" className=" shad-input" {...field} />
                </FormControl>
          ...
          />
          <Button className="shad-button_primary" type="submit">{ isLoading ? '注册...' : '注 册' }</Button>
          <p className="text-small-regular text-light-2 text-center mt-2">
            已有账号？
            <Link to={"/login"} className="text-primary-500 text-small-semibold ml-1">登录</Link>
        </p>
        </form>
      </Form>
  );
